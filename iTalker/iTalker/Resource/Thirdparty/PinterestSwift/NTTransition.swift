...
class NTTransition : NSObject , UIViewControllerAnimatedTransitioning{
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if presenting {
            ...
            var gridView = waterFallView.cellForItem(at: indexPath)
            
            if(gridView == nil) {
                waterFallView.scrollToItem(at: indexPath, at: .centeredVertically, animated: false)
                waterFallView.layoutIfNeeded()
                gridView = waterFallView.cellForItem(at: indexPath)
            }
